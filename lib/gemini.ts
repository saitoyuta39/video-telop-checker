...
【最重要：音声は無視すること】
音声（話している内容）は一切使わないでください。テロップの「文字」だけを見て判断してください。
- 音声とテロップの内容が異なっていても、それはエラーではありません。報告しないでください。
- テロップの文字だけを見て、その文字列に誤字脱字があるかどうかだけを判定してください。
- 「音声では〜と言っている」「音声と一致しない」といった理由での指摘は絶対にしないでください。
...
【最重要：濁点（゛）・半濁点（゜）の判定について】
濁点の有無だけが異なる文字（か↔が、た↔だ、は↔ば↔ぱ 等）は、動画の解像度やフォントによって非常に見分けにくいです。
あなたは濁点の有無を正確に判別できないことがあります。以下のルールを厳守してください：
- 濁点の有無だけが違う指摘（例：「何か」→「何が」、「何が」→「何か」）は、**報告しないでください**。
- 濁点の読み間違いはあなた自身の視覚的な誤認である可能性が極めて高いです。
- 同様に、半濁点（ぱ・ぴ・ぷ・ぺ・ぽ）の有無だけが異なる指摘も報告しないでください。
...
- 以下は演出・表現として許容し、エラーとして報告しないこと：
  ...
  - 口語表現（「〜してる」「〜だよね」等）
  - 強調表現（「すごーい」「めっちゃ」等）
- 指摘を出す前に、該当フレームに戻って文字を再確認すること（特に濁点・半濁点・促音・長音）。
- 「current_text」には画面に実際に表示されている文字列をそのまま入れること。自分で修正した文字列を入れないこと。
- current_text と suggested_text の違いが1文字だけの場合は、自分の読み取りミスの可能性を疑い、確信がなければ報告しないこと。
